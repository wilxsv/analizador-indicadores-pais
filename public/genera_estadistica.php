...
function getEstadisticaDelito($wpdb, $anyo, $vars){
  ...
  $data .= getDelitosPorSQL($wpdb, "SELECT sector_policial a, COUNT(IF(delito = 'AMENAZAS',1,NULL)) b, COUNT(IF(delito = 'EXPRESIONES DE VIOLENCIA CONTRA LA MUJER',1,NULL)) c, COUNT(IF(delito = 'EXTORSION',1,NULL)) d, COUNT(IF(delito = 'HOMICIDIO',1,NULL)) e, COUNT(IF(delito = 'HURTO',1,NULL)) f, COUNT(IF(delito = 'HURTO DE VEHICULOS',1,NULL)) g, COUNT(IF(delito = 'HURTO Y ROBO VEH/CM',1,NULL)) h, COUNT(IF(delito = 'LESIONES',1,NULL)) i, COUNT(IF(delito = 'ROBO',1,NULL)) j, COUNT(IF(delito = 'ROBO DE VEHICULOS',1,NULL)) k, COUNT(IF(delito = 'VIOLACION',1,NULL)) l, COUNT(IF(delito = 'VIOLENCIA INTRAFAMILIAR',1,NULL)) m FROM ind_bnc_delito WHERE anyo = $anyo AND $filtro GROUP BY sector_policial ORDER BY 1", "<tr><td>DELITO SEGUN SECTOR POLICIAL</td><td>AMENAZAS</td><td>EXPRESIONES DE VIOLENCIA CONTRA LA MUJER</td><td>EXTORSION</td><td>HOMICIDIO</td><td>HURTO</td><td>HURTO DE VEHICULOS</td><td>HURTO Y ROBO VEH/CM</td><td>LESIONES</td><td>ROBO</td><td>ROBO DE VEHICULOS</td><td>VIOLACION</td><td>VIOLENCIA INTRAFAMILIAR</td><td></td></tr>").getLinea($max);
  ...
  $data .= getDelitosPorSQL($wpdb, "SELECT sector_policial a, COUNT(IF(agresor_tipo_arma = 'ARMA BLANCA',1,NULL)) b, COUNT(IF(agresor_tipo_arma = 'ARMA DE FUEGO',1,NULL)) c, COUNT(IF(agresor_tipo_arma = 'CONTUNDENTE',1,NULL)) d, COUNT(IF(agresor_tipo_arma = 'FUERZA FISICA',1,NULL)) e, COUNT(IF(agresor_tipo_arma = 'LLAMADA - MENSAJES',1,NULL)) f, COUNT(IF(agresor_tipo_arma = 'ND',1,NULL)) g, COUNT(IF(agresor_tipo_arma = 'OTRAS',1,NULL)) h, '' i, '' j, '' k, '' l, '' m FROM ind_bnc_delito WHERE anyo = $anyo AND $filtro GROUP BY sector_policial ORDER BY 1", "<tr><td>SECTOR SEGUN ARMA</td><td>ARMA BLANCA</td><td>ARMA DE FUEGO</td><td>CONTUNDENTE</td><td>FUERZA FISICA</td><td>LLAMADA - MENSAJES</td><td>ND</td><td>OTRAS</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>").getLinea($max);
...
function getEstadisticaRetornados($wpdb, $anyo, $vars){
  ...
  $data .= getRetornadosPorSQL($wpdb, "SELECT mes AS u, COUNT(*) AS d, '' AS t, '' AS c ,
CASE
    WHEN mes = 'Enero' THEN 0
    WHEN mes = 'Febrero' THEN 1
    WHEN mes = 'Marzo' THEN 2
    WHEN mes = 'Abril' THEN 3
    WHEN mes = 'Mayo' THEN 4
    WHEN mes = 'Junio' THEN 5
    WHEN mes = 'Julio' THEN 6
    WHEN mes = 'Agosto' THEN 7
    WHEN mes = 'Septiembre' THEN 8
    WHEN mes = 'Octubre' THEN 9
    WHEN mes = 'Noviembre' THEN 10
    WHEN mes = 'Diciembre' THEN 11
    ELSE 12
END AS num_mes FROM ind_bnc_retornado WHERE anyo = $anyo AND $filtro GROUP BY mes ORDER BY 5", "<tr><td>Retornados por mes</td><td>Frecuencia</td><td></td><td></td></tr>").getLinea($max);
