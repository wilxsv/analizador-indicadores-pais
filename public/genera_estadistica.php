...
function getEstadisticaRetornados($wpdb, $anyo, $vars){
  ...
  $data .= getRetornadosPorSQL($wpdb, "SELECT mes AS u, COUNT(*) AS d, '' AS t, '' AS c FROM ind_bnc_retornado WHERE anyo = $anyo AND $filtro GROUP BY mes ORDER BY 1", "<tr><td>Retornados por mes</td><td>Frecuencia</td><td></td><td></td></tr>").getLinea($max);
