...
*/
 ...
 $file_name = "centroescolar.csv";
 $anyo = 0;

if(isset($_POST['importSubmitCE'])){
	...
	if(!empty($_FILES['input-file-now']['name']) && in_array($_FILES['input-file-now']['type'],$csvMimes)){
		if(is_uploaded_file($_FILES['input-file-now']['tmp_name'])){
          ...
          $wpdb->query("START TRANSACTION;");
            ...
            if ( is_numeric($line[0]) && is_numeric($line[4]) && is_numeric($line[5]) && is_numeric($line[7]) && is_numeric($line[8]) && is_numeric($line[9]) && is_numeric($line[10]) && is_numeric($line[11]) && is_numeric($line[12]) && is_numeric($line[13]) ) {
              $sql =  "INSERT INTO ind_centro_escolar (codigo, departamento, municipio, nombre_ce, lon, lat, sector, presencia_mara, drogas, violacion, armas, robos, matricula, anyo, registro, usuario, ip, hash) VALUES ";
  						$sql .= "($line[0], '$line[1]', '$line[2]', '$line[3]', $line[4], $line[5], '$line[6]', $line[7], $line[8], $line[9], $line[10], $line[11], $line[12],$line[13], '$date', $user, '$ip', '$hash')";
              $anyo = $line[13];
              $wpdb->query($sql);
              if($wpdb->last_error == ''){
                $total+=1;
...
<div class="row">
    ...
    <h2>Registro de indice de seguridad en Centros Escolares</h2>
	</div>
	<div class="col-md-8 ol-md-6 col-xs-12">
		<form  method="post" enctype="multipart/form-data" id="importFrm">
            ...
            <input type="submit" class="btn btn-primary" name="importSubmitCE" value="Importar archivo">
        </form>
	</div>
</div>
