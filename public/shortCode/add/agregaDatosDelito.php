...
*/
 ...
 $file_name = "delitos.csv";
 $head = "<tr><th>Departamento</th><th>Municipio</th><th>Sexo agresor</th><th>Delito</th><th>Año</th><th>Registro</th></tr>";
 $limit = 1000;
 $sql_grid = "SELECT departamento AS u, municipio AS d, agresor_sexo AS t, delito AS c, anyo AS i, registro AS s FROM ind_bnc_delito ORDER BY registro DESC LIMIT $limit";
 $id = uniqid();
	...
	if(!empty($_FILES['input-file-now']['name']) && in_array($_FILES['input-file-now']['type'],$csvMimes)){
		if(is_uploaded_file($_FILES['input-file-now']['tmp_name'])){
          ...
          $wpdb->query("START TRANSACTION;");
            ...
            if ( is_numeric($line[0]) && is_numeric($line[9]) ) {
              $sql =  "INSERT INTO ind_bnc_delito (cuenta, departamento, municipio, espacio, area, delito, dia, hora_rango, mes, anyo, victima_edad, victima_edad_rango_etario, victima_sexo, victima_ocupacion, agresor_relacion_victima, agresor_edad, agresor_sexo, agresor_tipo_arma, sector_policial, movil_hecho, registro, usuario, ip, hash) VALUES";
  						$sql .= "($line[0], '$line[1]', '$line[2]', '$line[3]', '$line[4]', '$line[5]', '$line[6]', '$line[7]', '$line[8]', $line[9], '$line[10]', '$line[11]', '$line[12]', '$line[13]', '$line[14]', '$line[15]', '$line[16]', '$line[17]', '$line[18]', '$line[19]', '$date', $user, '$ip', '$hash')";
  						$wpdb->query($sql);
              if($wpdb->last_error == ''){
                $total+=1;
                $linea+=1;
