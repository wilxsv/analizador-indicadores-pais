...
{	
	...
	function __construct(&$params) 
	{
		...
		$clickAction = $this->pSet->getClickActionJSONString();
		if( $clickAction != "" )
			$this->jsSettings['tableSettings'][$this->tName]["clickActions"] = my_json_decode( $clickAction );
	...
	function commonAssign() 
	{
		...
		if( ( $this->isDispGrid() || $this->listAjax && $this->pSet->noRecordsOnFirstPage() && !$this->searchClauseObj->isSearchFunctionalityActivated() ) 
			&& ( $this->printAvailable() ||  $this->exportAvailable() ||  $this->deleteAvailable() || $this->updateSelectedAvailable() 
				|| $this->inlineEditAvailable() || $this->inlineAddAvailable() || $this->showAddInPopup && $this->addAvailable() ) )
		{
	...
	protected function assignSortByDropdown()
	{
		...
		if( !$this->rowsFound )
		{
			if( $this->pSet->noRecordsOnFirstPage() && !$this->searchClauseObj->isSearchFunctionalityActivated() && $this->listAjax ) 
			{
