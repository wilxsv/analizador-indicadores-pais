...
{
	$icon = "";
	if( $dashName == "MINED IPCE" && $dashElementName == "Mined_Prueba_map" )
	{
		if($data["ipce_0_1"] <=0.32)
  $icon = "images/icons/esc_verde.png"; 
else if($data["ipce_0_1"] <= 0.66)
  $icon = "images/icons/amarillo.png"; 
else if($data["ipce_0_1"] <= 1)
  $icon = "images/icons/rojo.png"; 
else
  $icon = "";;
		return $icon;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="PPL por rango etario según género" && $field=="mun_resid")
	{
		return "SAN SALVADOR";
	}
	return "";
