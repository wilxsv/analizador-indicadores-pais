...
function checkTableName($shortTName, $type=false)
{
	...
	if ("Delitos_especificos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("carcel1" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("class_lugarespecifico" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("sectorppdf" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_mara" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("carcel_CAMBIOS" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("clasificacion" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Espacios_abiertos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechosdelictivos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechosdelictivos_lugaresp" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Todos_los_datos_PPL" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("PPL_pertenecientes_a_Pandilla" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Sector_PPD" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Homicidios_tentados_por_rango_etarios__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Hurtos_por_rango_etarios__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Hurtos_de_veh_culo_por_rango_etarios__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Lesiones_por_rango_etarios_etarios__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Robos_por_rango_etario__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Robo_de_veh_culo_por_rango_etario__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Robo_y_hurto_de_veh_culo_con_mercaderia_por_rango_etario__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Personas_privadas_de_libertad_por_Sectores_Estandarizados_seg_n_g_nero" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por__rea" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Personas_privadas_de_libertad_por_rango_etario_seg_n_g_nero" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Personas_privadas_de_libertad_por_rango_etario__segun_nivel_educativo_y_genero_Femenino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Personas_privadas_de_libertad_por_rango_etario__segun_nivel_educativo_y_genero_Masculino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Personas_privadas_de_libertad__segun_Sector_Estandarizado_y_pertenecientes_a_pandillas" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por_lugar_del_hecho" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por_M_vil" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Conteo_de_delitos_por_sector_estandarizado_seg_n_Movil" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por_tipo_de_arma" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Ocupaci_n_de_la_v_ctima_por_Delitos" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Amenazas__por_rango_etarios__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_Sexuales__por_rango_etarios__seg_n_sexo_victima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Homicidios_por_rango_etarios__seg_n_sexo" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por_sector_Estandarizado" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por_rango_de_horas" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Conteo_de_delitos__por_Sector_Estandarizado_seg_n_Movil" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Delitos_por_mes_del_hecho" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("homicidios_sv_2016" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("homicidios_sv_2016_Report" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("homicidios_sv_2016_Report1" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Muertes_PNC_enfrentamientos" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("homicidios_municipio" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Municipios_con_porcentaje_de_enfrentamiento" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_pertenecientes_a_pandillas_por_municipio" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_delitos" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Transito_class_Rango_Etario" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("PPL_delitos_clasificacion" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Carcel_hurto_mercaderia" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("PPL_educacion" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Personas_Privadas_de_Libertad" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("hechos_delictivos_class_delitos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Sector_PPD_por_pandilla_de_afiliacion__prueba_" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("tipo_organizacion_delictiva" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("carcel_visualizacion_todos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Grupos_etarios_por_estado_civil__masculino_" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Grupo_etarios_por_estado_civil__masculino_" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Grupo_etarios_por_estado_civil__femenino_" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_Grupos_etarios_por_afiliaci_n_a_Pandillas" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_Grupos_etarios_por_afiliaci_n_a_Pandillas_femenino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_grupos_etarios_por_Deportado_masculino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_grupos_etarios_por_Deportado_Femenino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_grupos_etarios_delito_genero_masculino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("hechos_delictivos1" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("PPL_Grupos_etarios_por_delitos_masculino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_Grupos_etarios_por_delitos_femenino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("DELITOS_PNC" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("carcel_prueba_santa_tecla" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("carcel_clasificacion_lugar" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mes_corregido" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Hechos_delictivos_dia_del_hecho" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("hechos_delictivos11" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("BD_Carcel" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("DB_PNC" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Hechosdelictivos_movil_class" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechosdelictivos_tipo_arma" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Municipios_y_delitos" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Hechosdelictivos_ocupacion_class" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechosdelictivos_relacion_vict_agresor" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Relacion_de_victima_con_agresor" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("hechosdelictivos_lugar_del_hecho" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pnc911" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pnc9111_class_delito" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pnc9111" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pnc911_Sector_por_eventos" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("pnc911municipios" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PNC911_Avisos" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("horas_completar" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hora_sustituir" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechos_delictivos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hechosdelictivos2" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("PPL_pertenecientes_a_Pandilla_prueba" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("PPL_pertenecientes_a_Pandilla_prueba1" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("retornados" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Retornados__clasificacion_rango_etario" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("BD_Retornados" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Regi_n_Repatriaci_n_por_a_o" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("mes_corregido_retornado" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Retornados_Regiones_repatriaci_n_por_meses" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Consulados_por_a_o" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Dashboard" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("Retornados_Departamento_por_a_o" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Municipios_por_a_o" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Rango_etario_por_sexo" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Rango_etario_por_antecedentes_g_nero_masculino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Retornados_Rango_etario_por_antecedentes_g_nero_femenino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("transito" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Tr_nsito__Departamento_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("Tr_nsito__municipio_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Fallecidos_Rango_etario_v_ctima_por_sexo_v_ctima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Lesionados_Rango_etario_v_ctima_por_sexo_v_ctima" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Calidad_de_la_v_ctima_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Tipo_de_accidente_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Transito_class_causas" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("transito1" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Tr_nsito_Causa_del_accidente_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Transito_class_tipo_veh_culo" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Tr_nsito_Tipo_de_veh_culo_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Ruta_transporte_colectivo_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Responsable_accidente_detenido_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Transito_extraer_mes" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Tr_nsito_Resultado_del_accidente_por_meses_de_accidentes" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Resultado_del_accidente_por_rango_horario_de_accidentes" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Tr_nsito_Direcci_n_accidente_por_resultado_del_accidente" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Transito_prueba_datos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("empalme_tablas" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechos_delictivos12" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Municipios" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Transito_arreglar_guiones" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("sectorppd1" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Sector_PPD_24" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("sectorppd1_Report" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Dashboard" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("sectorppd1_Chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("Hechosdelictivos_rango_etario" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("carcel_Report" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("pnc9112_PRUEBA" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Sector_PPD_por_tipo_de_arma" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Cruce_Sector_PPD" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("enlazando_sector" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Retornados_Todos_los_departamentos" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("reemplazar_sectores" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("hechos_delictivos_VISTA" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("reemplazar_sectores_PPL" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("anexar_carcel" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("anexar_carcel1" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("SUPRIMIR_REGISTROS_ANEXO_CARCEL" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Sectorizacion" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("PPL_DGCP" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("Personas_Privadas_de_Libertad123" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("Personas_privadas_de_libertad_por_rango_etario__segun_nivel_educativo_y_g_nero_femenino" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Dashboard1" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("PPL_por_rango_etario_seg_n_g_nero" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("reemplazar_sectores_hechosdelictivos" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("mined1" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined2" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined21" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined_Report" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("MINED_IPCE" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("MINED_INDICE" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined11" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("calcular_indice_MINED" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("calcular_indice_MINED2" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined1_Report" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Maxima_Matricula_Mined" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Matricula_Relativa_Final" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("IPCE_NAIVE" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("IPCE_FINAL" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Mined_Maximo_IPCE" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined_maximo_valor" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("BD_Mined" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined3" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("valor_maximo_mined_2" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("BD_SECTORES" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Anexar_Sector_a_PPL" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("mined4" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("anexar_carcel2" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("BD_ANEXO_CARCEL" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	$arr = array();
		...
		$arr[]="PPL pertenecientes a Pandilla";
		$arr[]="Sector PPD";
		$arr[]="Homicidios tentados por rango etarios, según sexo victima";
		$arr[]="Hurtos por rango etarios, según sexo victima";
		$arr[]="Hurtos de vehículo por rango etarios, según sexo victima";
		$arr[]="Lesiones por rango etarios etarios, según sexo victima";
		$arr[]="Robos por rango etario, según sexo victima";
		$arr[]="Robo de vehículo por rango etario, según sexo victima";
		$arr[]="Robo y hurto de vehículo con mercaderia por rango etario, según sexo victima";
		$arr[]="Personas privadas de libertad por Sectores Estandarizados según género";
		$arr[]="Delitos por área";
		$arr[]="Personas privadas de libertad por rango etario según género";
		$arr[]="Personas privadas de libertad por rango etario, segun nivel educativo y genero Femenino";
		$arr[]="Personas privadas de libertad por rango etario, segun nivel educativo y genero Masculino";
		$arr[]="Personas privadas de libertad  segun Sector Estandarizado y pertenecientes a pandillas";
		$arr[]="Delitos por lugar del hecho";
		$arr[]="Delitos por Móvil";
		$arr[]="Conteo de delitos por sector estandarizado según Movil";
		$arr[]="Delitos por tipo de arma";
		$arr[]="Ocupación de la víctima por Delitos";
		$arr[]="Amenazas, por rango etarios, según sexo victima";
		$arr[]="Delitos Sexuales  por rango etarios, según sexo victima";
		$arr[]="Homicidios por rango etarios, según sexo";
		$arr[]="Delitos por sector Estandarizado";
		$arr[]="Delitos por rango de horas";
		$arr[]="Conteo de delitos, por Sector Estandarizado según Movil";
		$arr[]="Delitos por mes del hecho";
		$arr[]="homicidios_sv_2016";
		$arr[]="homicidios_sv_2016 Report";
		$arr[]="homicidios_sv_2016 Report1";
		$arr[]="Muertes PNC enfrentamientos";
		$arr[]="homicidios_municipio";
		$arr[]="Municipios con porcentaje de enfrentamiento";
		$arr[]="PPL pertenecientes a pandillas por municipio";
		$arr[]="PPL delitos";
		$arr[]="Transito class Rango Etario";
		$arr[]="PPL delitos_clasificacion";
		$arr[]="Carcel_hurto_mercaderia";
		$arr[]="PPL_educacion";
		$arr[]="Personas Privadas de Libertad";
		$arr[]="hechos_delictivos_class delitos";
		$arr[]="Sector PPD por pandilla de afiliacion (prueba)";
		$arr[]="tipo organizacion delictiva";
		$arr[]="carcel visualizacion todos";
		$arr[]="Grupos etarios por estado civil (masculino)";
		$arr[]="Grupo etarios por estado civil (masculino)";
		$arr[]="Grupo etarios por estado civil (femenino)";
		$arr[]="PPL Grupos etarios por afiliación a Pandillas";
		$arr[]="PPL Grupos etarios por afiliación a Pandillas femenino";
		$arr[]="PPL grupos etarios por Deportado masculino";
		$arr[]="PPL grupos etarios por Deportado Femenino";
		$arr[]="PPL grupos etarios delito genero masculino";
		$arr[]="hechos_delictivos1";
		$arr[]="PPL Grupos etarios por delitos masculino";
		$arr[]="PPL Grupos etarios por delitos femenino";
		$arr[]="DELITOS PNC";
		$arr[]="carcel prueba santa tecla";
		$arr[]="carcel_clasificacion lugar";
		$arr[]="mes_corregido";
		$arr[]="Hechos delictivos dia del hecho";
		$arr[]="hechos_delictivos11";
		$arr[]="BD Carcel";
		$arr[]="DB PNC";
		$arr[]="Hechosdelictivos movil class";
		$arr[]="hechosdelictivos_tipo_arma";
		$arr[]="Municipios y delitos";
		$arr[]="Hechosdelictivos ocupacion class";
		$arr[]="hechosdelictivos_relacion_vict_agresor";
		$arr[]="Relacion de victima con agresor";
		$arr[]="hechosdelictivos_lugar_del_hecho";
		$arr[]="pnc911";
		$arr[]="pnc9111 class delito";
		$arr[]="pnc9111";
		$arr[]="pnc911 Sector por eventos";
		$arr[]="pnc911municipios";
		$arr[]="PNC911 Avisos";
		$arr[]="horas_completar";
		$arr[]="hora_sustituir";
		$arr[]="hechos_delictivos";
		$arr[]="hechosdelictivos2";
		$arr[]="PPL pertenecientes a Pandilla_prueba";
		$arr[]="PPL pertenecientes a Pandilla_prueba1";
		$arr[]="retornados";
		$arr[]="Retornados, clasificacion rango etario";
		$arr[]="BD Retornados";
		$arr[]="Retornados-Región Repatriación por año";
		$arr[]="mes corregido retornado";
		$arr[]="Retornados-Regiones repatriación por meses";
		$arr[]="Retornados-Consulados por año";
		$arr[]="Retornados Dashboard";
		$arr[]="Retornados-Departamento por año";
		$arr[]="Retornados-Municipios por año";
		$arr[]="Retornados-Rango etario por sexo";
		$arr[]="Retornados-Rango etario por antecedentes género masculino";
		$arr[]="Retornados-Rango etario por antecedentes género femenino";
		$arr[]="transito";
		$arr[]="Tránsito- Departamento por resultado del accidente";
		$arr[]="Tránsito";
		$arr[]="Tránsito- municipio por resultado del accidente";
		$arr[]="Tránsito-Fallecidos Rango etario víctima por sexo víctima";
		$arr[]="Tránsito-Lesionados Rango etario víctima por sexo víctima";
		$arr[]="Tránsito-Calidad de la víctima por resultado del accidente";
		$arr[]="Tránsito-Tipo de accidente por resultado del accidente";
		$arr[]="Transito class causas";
		$arr[]="transito1";
		$arr[]="Tránsito-Causa del accidente por resultado del accidente";
		$arr[]="Transito class tipo vehículo";
		$arr[]="Tránsito-Tipo de vehículo por resultado del accidente";
		$arr[]="Tránsito-Ruta transporte colectivo por resultado del accidente";
		$arr[]="Tránsito-Responsable accidente detenido por resultado del accidente";
		$arr[]="Transito extraer mes";
		$arr[]="Tránsito-Resultado del accidente por meses de accidentes";
		$arr[]="Tránsito-Resultado del accidente por rango horario de accidentes";
		$arr[]="Tránsito-Dirección accidente por resultado del accidente";
		...
		$arr[]="sectorppd1";
		$arr[]="Sector PPD 24";
		$arr[]="sectorppd1 Report";
		$arr[]="Dashboard";
		$arr[]="sectorppd1 Chart";
		$arr[]="Hechosdelictivos_rango etario";
		$arr[]="carcel Report";
		$arr[]="pnc9112 PRUEBA";
		$arr[]="Sector PPD por tipo de arma";
		$arr[]="Cruce Sector PPD";
		$arr[]="enlazando sector";
		$arr[]="Retornados-Todos los departamentos";
		...
		$arr[]="Sectorizacion";
		$arr[]="PPL DGCP";
		$arr[]="Personas Privadas de Libertad123";
		$arr[]="Personas privadas de libertad por rango etario, segun nivel educativo y género femenino";
		$arr[]="Dashboard1";
		$arr[]="PPL por rango etario según género";
...
{
	if($shorttable=="sectorppd1_Chart")
		return "2DColumn";
	return "";
}
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("carcel")
