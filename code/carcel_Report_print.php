...
require_once("include/carcel_Report_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "sandstone";
		$layout->customCssPageName = "carcel_Report_rprint";
...
$page_layouts["carcel_Report_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=carcel_Report.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=carcel_Report.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
