...
require_once("include/BD_Retornados_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "sandstone";
		$layout->customCssPageName = "BD_Retornados_rprint";
...
$page_layouts["BD_Retornados_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=BD_Retornados.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=BD_Retornados.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
